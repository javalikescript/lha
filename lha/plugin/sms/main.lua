...
local FreeMobileSms = class.create(function(freeMobileSms)
  ...
  function freeMobileSms:getUrl()
    return apiUrl..'?user='..self.user..'&pass='..self.pass..'&msg='..msg
  end
...
logger:info('FreeMobileSms user: "'..configuration.user..'"')

plugin:watchDataValue(plugin:getPath('sms'), function(value, previousValue, path)
